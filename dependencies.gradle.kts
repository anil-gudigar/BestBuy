...
fun getEnvironmentName(): String {
        ...
        for (taskName in taskNames) {
                return when {
                        taskName.contains("Debug", ignoreCase = true) -> appProperties["DEBUG_WORKING_ENV"]!!.toString().toLowerCase().replace("\"", "")
                        taskName.contains("Release", ignoreCase = true) -> appProperties["WORKING_ENV"]!!.toString().toLowerCase().replace("\"", "")
                        else -> "env"
                }
        }
        return "env"
...
fun getVersionFileNameDependsOnBuildType(): String {
        ...
        for (taskName in taskNames) {
                return when {
                        ...
                        taskName.contains("Release", ignoreCase = true) -> extra["releaseVersionFileName"] as String
...
val appVersionName = readVersionName(getVersionFileNameDependsOnBuildType(), getCurrentTaskName(), getEnvironmentName())
